# Analyze hydrologic signature results for Arizona watersheds
...
setwd('/Volumes/GoogleDrive/Shared drives/Lauren and Hilary/Regional Overland Flow Project/Data and Scripts/Arizona/Signature Data/')
...
p_db_SE_thresh <- ggplot()+
  ...
  geom_point(data = diff_df, aes(x = mons_SE_thresh, y = site), size = 2, color = 'deepskyblue3',
             shape = ifelse(diff_df$mons_SE_thresh_signif < 0.05, 17, 16))+
  geom_point(data = diff_df, aes(x = SE_thresh, y = site), size = 2, color = 'darkgoldenrod1',
             shape = ifelse(diff_df$SE_thresh_signif < 0.05, 17, 16))+
  # xlim(0,100)+
